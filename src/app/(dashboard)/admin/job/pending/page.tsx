...
export default function PendingJobsBody() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/jobs?status=pending`);
  ...
  if (jobs.length === 0) {
    return <div className="text-center py-6 text-slate-400">No pending job posts found.</div>;
  }
      ...
      {jobs.map((job) => (
        ...
        >
          ...
          <div className="flex gap-2">
            ...
            <button className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded font-medium">
              Reject
            </button>
          </div>
        </div>
