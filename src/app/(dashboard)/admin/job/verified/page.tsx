...
export default function VerifiedJobsBody() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        console.error("Failed to fetch verified jobs:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (jobs.length === 0) {
    return <div className="text-center py-6 text-slate-400">No verified job posts found.</div>;
  }
