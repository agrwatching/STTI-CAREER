...
export default function Job() {
  // Stat management
  const [jobs, setJobs] = useState([]); // menyimpan data lowongan pekerjaan
  ...
  // 3. PAGINATION LOGIC
  // -------------------------
  const totalPages = Math.ceil(jobs.length / itemsPerPage); // total halaman
  const startIdx = (page - 1) * itemsPerPage; // index awal
  const currentJobs = jobs.slice(startIdx, startIdx + itemsPerPage); // data per halaman
  ...
  return (
    <section className="min-h-screen px-8 py-12 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {currentJobs.map((job) => (
          <div key={job.id} className="bg-white p-6 rounded-2xl shadow hover:shadow-lg flex flex-col justify-between">
            {/* Header */}
            <div className="flex items-start gap-3 mb-4">
              {/* API tidak ada logo → pakai dummy logo */}
              <img src="https://dummyimage.com/60x60/2563eb/ffffff&text=C" alt="company logo" className="w-12 h-12 rounded-full object-cover" />
              <div>
                <h3 className="text-lg font-semibold">{job.job_title}</h3>
                <p className="text-gray-600 text-sm">Company #{job.company_id}</p>
              </div>
            </div>

            {/* Description */}
            <p className="text-gray-500 text-sm mb-4 line-clamp-3">{job.job_description}</p>

            {/* Tags */}
            <div className="flex gap-2 flex-wrap mb-2 ml-2">
              <span className="text-xs bg-blue-100 text-blue-700 px-3 py-1 rounded-full">Fulltime (work mode, masih statis tidak ada di api backend)</span>
              <span className="text-xs bg-green-100 text-green-700 px-3 py-1 rounded-full">Remote (work mode, masih statis tidak ada di api backend)</span>
            </div>

            {/* Salary */}
            <div className="">
              <span className="text-xs bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full">
                {job.salary_min != null && job.salary_max != null ? `Rp ${(job.salary_min ?? 0).toLocaleString()} - Rp ${(job.salary_max ?? 0).toLocaleString()}` : "Salary not specified"}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <Link href={`/lowongan/${job.id}`} className="bg-blue-600 text-white text-sm px-4 py-2 rounded-full hover:bg-blue-700">
                Apply Now
              </Link>
            </div>
          </div>
